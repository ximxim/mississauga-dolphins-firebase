...
SlackMessages.prototype.message = async(text => {
  const slackMessage = {
    username: "Google Cloud",
    text
  };
...
SlackMessages.prototype.newsFeedItemMessage = async(
  (item, responseUrl, buttons) => {
  ...
  if (item.attachments) {
    _.map(item.attachments.data, attachment => {
      if (attachment.type !== "album") {
        urls += attachment.media.image
          ? `<${attachment.media.image.src} |Facebook Attachment>  \n`
          : "";
      }
    });
  }
